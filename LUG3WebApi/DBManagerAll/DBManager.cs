...
namespace LUG3WebApi.DBManagerAll 
{
    public class DBManager : IDBManager 
    {
        ...
        {
            var sql = "INSERT INTO Meeting (IdInstance, IdPostulant, DateTime) VALUES (@Idinstance, @IdPostulant, @DateTime)";
        ...
        public IEnumerable <ResultDetailsView> GetResultsforView(int IdPostulant){
             var sql = "SELECT r.Id, m.IdInstance, r.IdMeeting, r.Name, r.Observation, r.OK FROM Result r INNER JOIN Meeting m ON (m.Id = r.IdMeeting) WHERE m.IdPostulant = @IdPostulant";

            IEnumerable <ResultDetailsView> result;
            using(var conn = new SqlConnection(connStr))
            {
                conn.Open();
                result = conn.Query<ResultDetailsView>(sql, new{ @IdPostulant = IdPostulant });
                conn.Close();
            }
            return result;
        }
